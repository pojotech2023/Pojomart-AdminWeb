...
<div class="product-card card product-card-clickable" data-product-id="{{ $product->id }}">
    ...
    @if (!empty(json_decode($product['image'],true)))
        <img src="{{ $product->identityImageFullPath[0] ?? asset_path('assets/admin/img/160x160/2.png') }}" class="w-100 h-100 object-cover aspect-ratio-80">
    @else
        <img src="{{asset_path('assets/admin/img/160x160/2.png')}}" class="w-100 h-100 object-cover aspect-ratio-80"
        >
    @endif
    </div>
