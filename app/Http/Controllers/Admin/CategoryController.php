...
class CategoryController extends Controller
{
    ...
    function index(Request $request): View|Factory|Application
    {
        ...
        $categoryLimit = $this->planLimitService->getLimit('category_limit', 9);
        ...
        $categoryCount = $this->category->where(['position' => 0])->count();

        return view('admin-views.category.index', compact('categories', 'search', 'categoryLimit', 'categoryCount'));
    }
    ...
    function subIndex(Request $request): View|Factory|Application
    {
        $queryParam = [];
        $search = $request['search'];
        $subCategoryLimit = $this->planLimitService->getLimit('sub_category_limit', 8);
        if ($request->has('search')) {
            $key = explode(' ', $request['search']);
        ...
        $subCategoryCount = $this->category->where(['position' => 1])->count();
        return view('admin-views.category.sub-index', compact('categories', 'search', 'subCategoryLimit', 'subCategoryCount'));
    }
    ...
    function store(Request $request): RedirectResponse
    {
        ...
        $isMainCategory = (int)$parentId === 0 && (int)$request->position === 0;
        $isSubCategory = (int)$request->position === 1;
        ...
        if ($isMainCategory) {
            ...
            if ($categoryCount >= $categoryLimit) {
                return back()->with('category_limit_reached', true);
            }
        }
        ...
        if ($isSubCategory) {
            ...
            if ($subCategoryCount >= $subCategoryLimit) {
                return back()->with('sub_category_limit_reached', true);
            }
        }
