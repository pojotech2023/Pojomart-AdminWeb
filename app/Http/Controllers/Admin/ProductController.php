...
class ProductController extends Controller
{
    ...
    {
        $productLimit = $this->planLimitService->getLimit('product_limit', 4);
        $productCount = $this->product->count();

        if ($productCount >= $productLimit) {
            return redirect()->route('admin.product.list')->with('product_limit_reached', true);
        }
    ...
    public function list(Request $request): View|Factory|Application
    {
        ...
        if ($request->filled('category_id')) {
            $query = $query->where('category_ids', 'like', '%"id":"' . $categoryId . '","position":1%');
        }

        if ($request->filled('sub_category_id')) {
            $query = $query->where('category_ids', 'like', '%"id":"' . $subCategoryId . '","position":2%');
        }
        ...
        foreach ($products as $product) {
            $product->total_sold = $product->order_details
                ->filter(function ($detail) {
                    return optional($detail->order)->order_status === 'delivered';
                })
                ->sum('quantity');
        }
        ...
        return view('admin-views.product.list', compact('products', 'search', 'productLimit', 'productCount', 'categories', 'categoryId', 'subCategoryId'));
    }
    ...
    public function store(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        if ($productCount >= $productLimit) {
            return response()->json([
                'errors' => [[
                    'code' => 'product_limit',
                    'message' => 'You have reached the limit to add more product.'
                ]]
            ], 422);
        }
