<div class="product-card card quick-view-trigger" data-product-id="{{ $product->id }}">
    <?php
        $category_id = null;
        foreach (json_decode($product['category_ids'], true) as $cat) {
            if ($cat['position'] == 1){
                $category_id = ($cat['id']);
            }
        }

        $category_discount = \App\CentralLogics\Helpers::category_discount_calculate($category_id, $product['price']);
        $product_discount = \App\CentralLogics\Helpers::discount_calculate($product, $product['price']);

        if ($category_discount >= $product['price']){
            $discount = $product_discount;
        }else{
            $discount = max($category_discount, $product_discount);
        }
    ?>

    <div class="card-header inline_product clickable p-0" style="height: 160px; overflow: hidden;">
        @php
            $imagePath = $product->identityImageFullPath[0] ?? null;
            $imagePath = $imagePath ? str_replace('app/public/', '', $imagePath) : null;
            $isExternal = $imagePath && (Str::startsWith($imagePath, 'http://') || Str::startsWith($imagePath, 'https://'));
            $finalImage = $isExternal ? $imagePath : asset('storage/' . $imagePath);
        @endphp
        @if (!empty(json_decode($product['image'],true)))
            <img src="{{ $finalImage }}" style="width: 100%; height: 100%; object-fit: cover;" alt="{{ $product['name'] }}">
        @else
            <img src="{{ asset('assets/admin/img/160x160/2.png') }}" style="width: 100%; height: 100%; object-fit: cover;" alt="No Image">
        @endif
    </div>

    <div class="card-body inline_product text-center p-1 clickable">
        <div class="product-title1 text-dark font-weight-bold">
            {{ Str::limit($product['name'], 12) }}
        </div>
        <div class="justify-content-between text-center">
            <div class="product-price text-center">
                {{ Helpers::set_symbol($product['price'] - $discount) }}
            </div>
        </div>
    </div>
</div>
